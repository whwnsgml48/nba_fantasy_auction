...
🔴 **base를 전면 교체한 코어다.** 구 c2(Şengün·Gobert·THJ·Nembhard·Merrill)는 실측 79.2%로 7코어 최하위였다. 실제 12팀 목적함수로 재탐색해 **89.3% / 최저 75.0%** 안을 얻었고 평가 세션이 독립 재현(3시드 전부 c6 초과)으로 승인했다. **공식 시뮬 재실행(6000시행 · seed 20261020) 결과 88.9% / 최저 74.1% 로 전 코어 1위다** — c6 87.5% / 71.6%. 다만 조건부이므로 기본값은 여전히 c6다.
