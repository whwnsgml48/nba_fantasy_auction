...
🔴 **44차**: c6 BN 1순위가 Desmond Bane → **Jamal Murray** 로 바뀌었다(+2.63%p · 5.2σ · 감축① 후 +3.10%p). 위 「상위 다섯은 1.2%p 안이라 승률로 못 고른다」는 **더 이상 참이 아니다** — c6 가 2위와 2.67%p 벌어졌다(`core_selection_hierarchy_42.amended_44`). ⚠️ 단 그 우위는 **Murray 를 $31 이하에 잡을 때만** 성립한다. 못 잡으면 Bane → Brunson 으로 내려가고 c6 는 87.9% 로 돌아간다 — **목적지는 그래도 c6 다.** 판단표는 안 바뀐다.**
