...
> **정상 시장: 코어 6 기본 → Hali 할인 시 코어 1 → SGA 할인 시 코어 3 → 앵커 실패 시 코어 4. 저가 센터 2명 이상 과열: 즉시 코어 7. 🔴 **Jokić를 $97 이하로 잡으면 즉시 코어 2** — ~~상위 5개와 동급이고~~ **가정이 없다**(1순위 전원 직전 시즌 실측). ⚠️ **44차 정정: 「동급」은 더 이상 참이 아니다** — c2 86.8% vs c6 **90.4%** 로 c6 가 앞선다. c2 의 값은 **동급이라서가 아니라 「가정이 없다 + 조달 배율 1.06 (유일하게 예산 안에 들어온다)」** 때문이다. 그 둘은 그대로 유효하다. Sabonis는 조건 충족 시에만 별도 진입.
⚠️ 상위 다섯(c3·c6·c1·c7·c2)은 승률이 1.2%p 안(대응 오차 ±0.64%p)이라 **승률로 못 고른다** — **1차 각 행의 조건 · 2차 조달 · 3차 상위 8팀 상대 승률** 순으로 고를 것. 「가정 취약성」과 「최저 승률」은 **바닥 지표라 참고**다(8팀/14팀 진출 · 42차).
🔴 **44차**: c6 BN 1순위가 Desmond Bane → **Jamal Murray** 로 바뀌었다(**+2.57%p · 5.0σ · 감축① 후 +3.02%p** — 46차 DD 실계수 채택 후 재측정 · 44차 값은 +2.63/+3.10). 위 「상위 다섯은 1.2%p 안이라 승률로 못 고른다」는 **더 이상 참이 아니다** — c6 가 2위와 **2.29%p** 벌어졌다(대응 SE **0.67%p** 의 3.4배 · 46차 DD 채택 후 값)(`core_selection_hierarchy_42.amended_44`). ⚠️ 단 그 우위는 **Murray 를 $31 이하에 잡을 때만** 성립한다. 못 잡으면 Bane → Brunson 으로 내려간다(되돌림 승률은 46차에 재측정 — `docs/05 §20`) — **목적지는 그래도 c6 다.** 판단표는 안 바뀐다.**
...
| **2** | 센터 정상가 + Hali 불확실/비쌈 | **C6** — A/T 분산 조달 (정상 시장 기본값) | 정상 시장의 기본값. Hali가 기대만큼 안 싸거나 아킬레스 복귀 리스크를 피하고 싶으면 여기. 🔴 **Sabonis 가 방에서 헐값에 흐르면 — 그래도 여기(c6)다. 그를 무시하라.** 2026-08-31 에 c5(Sabonis 할인 플랜)를 판단표에서 내렸으므로 그 상황의 참조 플랜이 없어졌다. 답은 「c6 로 가고 Sabonis 는 안 산다」이고, 이유는 **그의 가격이 c6 를 막지 않기 때문**이다 — c6 는 그를 쓰지 않는다. 싸다는 것만으로 사면 가정 셋을 얹은 플랜(c5 · 대칭 스트레스에서 최하위)으로 끌려간다. |
...
| **조건부 ★** | Jokić ≤ $97 낙찰 → 즉시 코어 2 | **C2** — Jokić 압축 | 🔴 **게이트가 열리면 조립 가능성 순서를 앞선다** — ~~상위 5개와 **동급**이고~~ **가정 취약성이 ±0**이다\n⚠️ **44차 정정: 「동급」이 아니다.** (46차 DD 실계수 채택 후 재측정) c6 **90.4%** · c3 88.1% · c1 88.0% · c2 86.8% · c7 86.8% (대응 SE **0.67%p**) — **c6 가 c2 보다 3.6%p 앞선다.** 그래도 이 게이트를 유지하는 이유는 **승률이 아니라** ① 가정 취약성 ±0 ② 조달 배율 **1.06** — 일곱 중 유일하게 작년 가격으로 $200 안에 들어온다. 🔴 즉 c2 는 「c6 만큼 강해서」가 아니라 **「c6 를 못 사게 됐을 때 실제로 살 수 있어서」** 값이 있다. 그 근거는 44차에도 그대로다. — 1순위 아홉 명 전원이 직전 시즌 실측 위에 있어 프리시즌을 몰라도 안 흔들린다. 📌 이전에 여기 적혀 있던 **「88.9%로 전 코어 1위」는 철회됐다.** 그 값은 포지션 자격을 모르는 모델이 만든 것으로, C 전용 5명을 매일 다 쓸 수 있다고 계산했다. 자격을 반영하니 c2만 −2.4%p 움직였다(나머지 여섯은 ±0.4%p 안). ⏱ **Jokić는 최상위 선수라 드래프트 초반에 지명된다 — 즉 이 게이트의 개폐를 초반에 알게 된다.** 그 시점에 $97 이하로 잡았다면 기본값 c6로 가지 말고 c2로 전환하라. |
...
|---|---|---|---|---|---|---|---|---|---|
| c1 | 93.2% | 52.1% | 71.9% | 59.3% | 91.0% | 54.6% | **52.1%** | 가치최대 | 17.2% |
| c6 | 94.7% | 50.8% | 65.9% | 65.5% | 94.2% | 49.5% | **49.5%** | 벤치마크 | 35.3% |
| c7 | 92.0% | 48.6% | 68.7% | 58.4% | 90.8% | 53.6% | **48.6%** | 가치최대 | 16.2% |
| c3 | 92.2% | 45.6% | 67.8% | 60.8% | 92.7% | 50.4% | **45.6%** | 가치최대 | 20.2% |
| c5 | 90.5% | 44.3% | 64.4% | 57.6% | 87.4% | 47.2% | **44.3%** | 가치최대 | 18.6% |
| c2 | 90.6% | 48.4% | 63.3% | 64.5% | 91.1% | 43.9% | **43.9%** | 벤치마크 | 58.0% |
| c4 | 90.6% | 40.6% | 61.4% | 56.1% | 88.6% | 47.2% | **40.6%** | 가치최대 | 20.7% |
...
**주간 승률** 최소 **52.1%** (vs 가치최대) · 빅5 동시붕괴 17.2% · 기대 승리 캣 9.01
...
> Jokić 한 칸이 빅맨 캣(REB 12.8 · OREB 3.0 · DD)과 가드 캣(AST 10.5 · A/T +0.241)을 동시에 주므로, 나머지 8칸을 **전부 저가 구간**에서 채운다. $39 Derrick White 하나만 중가이고 $22 Bane 아래로는 전원 $12 이하다.
...
**주간 승률** 최소 **43.9%** (vs 벤치마크) · 빅5 동시붕괴 58.0% · 기대 승리 캣 9.18
...
**주간 승률** 최소 **45.6%** (vs 가치최대) · 빅5 동시붕괴 20.2% · 기대 승리 캣 8.74
...
**주간 승률** 최소 **40.6%** (vs 가치최대) · 빅5 동시붕괴 20.7% · 기대 승리 캣 8.51
...
**주간 승률** 최소 **44.3%** (vs 가치최대) · 빅5 동시붕괴 18.6% · 기대 승리 캣 8.73
...
**주간 승률** 최소 **49.5%** (vs 벤치마크) · 빅5 동시붕괴 35.3% · 기대 승리 캣 9.11
...
| **UTIL** | $12 | $20 | — | STL 2.0 리그 공동 1위 | **Dyson Daniels** | Ausar Thompson $3 | Cason Wallace $3 |
| **BN** | $22 | $31 | — | 전방위 저가 윙/가드 — 후보 **111명 실측 1위**(data/c6_promote.json · Bane 대비 +2.16%p) | **Jamal Murray** | Desmond Bane $22 | Jalen Brunson $23 |
...
**피벗 최종 9인** — 총액 $188 · 예비비 $12 · 빅맨 $76 (C자격 3명) · 노리는 캣 `3PM A/T AST BLK DD FG% FT% OREB PTS REB STL` · 포기 `3P% TOV`
...
**주간 승률** 최소 **48.6%** (vs 가치최대) · 빅5 동시붕괴 16.2% · 기대 승리 캣 8.88
