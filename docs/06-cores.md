...
| **조건부 ★** | Jokić ≤ $97 낙찰 → 즉시 코어 2 | **C2** — Jokić 압축 | 🔴 **게이트가 열리면 조립 가능성 순서를 앞선다** — 실측 86.5% / 최저 72.5% 로 상위 5개와 **동급**이고(c3 87.7 · c6 87.6 · c1 87.0 · c2 86.5 · c7 86.4, 폭 1.3%p · 코어 비교의 대응 오차 0.59%p) **가정 취약성이 ±0**이다 — 1순위 아홉 명 전원이 직전 시즌 실측 위에 있어 프리시즌을 몰라도 안 흔들린다. 📌 39차까지 여기 적혀 있던 **「88.9%로 전 코어 1위」는 철회됐다.** 그 값은 포지션 자격을 모르는 모델이 만든 것으로, C 전용 5명을 매일 다 쓸 수 있다고 계산했다. 자격을 반영하니 c2만 −2.4%p 움직였다(나머지 여섯은 ±0.4%p 안). ⏱ **Jokić는 최상위 선수라 드래프트 초반에 지명된다 — 즉 이 게이트의 개폐를 초반에 알게 된다.** 그 시점에 $97 이하로 잡았다면 기본값 c6로 가지 말고 c2로 전환하라. |
