...
왜 재는가 🔴
  🔴 2026-09-01 정정 — 드래프트는 2026-10-05, 프리시즌 경기 개시는 10-03 이다.
  이 스크립트가 쓰인 전제(「4.4주 앞서 확인 불가」)는 틀렸다. 다만 결론은 대체로 남는다:
  캠프 6일 + 실경기 최대 이틀로는 **「뛰는가」만** 알 수 있고 **실출장 수는 못 잰다**.
  (원문) 드래프트가 프리시즌보다 앞서 **복귀 여부를 확인할
...
def main():
    ...
    out = {"seed": SEED, "iterations": ITERS, "healthy_season": HEALTHY_SEASON,
           ...
           "why": ("2026-09-01 정정: 드래프트 2026-10-05 · 프리시즌 개시 10-03. "
                   "확인되는 것은 「뛰는가」 이항뿐이고 실출장 수는 못 잰다. "
