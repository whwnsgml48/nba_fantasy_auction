...
🔴 저장값과 새 측정을 **빼지 마라** (2026-09-01 · docs/11 ⑬)
  `team_week_prepped` 는 선수 리스트를 **순서대로** 돌며 난수를 뽑는다. 그래서
  **같은 로스터·같은 시드라도 이름 순서가 다르면 값이 달라진다.**
      원래 87.632 · 정렬 87.692 · 역순 87.677 · 셔플 87.571  → 폭 **0.121%p**
      대응 SE 0.59%p 의 **20%**
  저장된 승률은 각자 **그때의 이름 순서**로 만들어졌다. 오늘 잰 값에서 그걸 빼면
  차이 안에 「어떻게 만들었는지」가 섞인다 — 승격·조달 판정은 전부 SE 안에서 갈린다.

  **규약: 비교는 양쪽을 같은 규약으로 새로 잰다. 저장값은 표시용이다.**
  (`tool/pivot_delta.canon()` 처럼 이름을 정렬해 넘기면 measure 가 집합의 순수 함수가 된다.)

  ⚠️ 저장값을 다시 계산하지는 않는다 — `docs` 곳곳에 인용돼 있어 0.12%p 때문에 40차치를
     전부 다시 쓰는 것은 손해다. **고치는 것은 비교 방식이지 저장된 숫자가 아니다.**
  ⚠️ 그리고 이건 순서 결함 하나의 이야기가 아니다. **이 하네스를 고치면 저장값은 전부
     옛 규약이 된다.** 다음에 여기를 고치는 사람은 이 문단을 먼저 읽을 것.
