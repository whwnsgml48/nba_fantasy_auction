...
월요일에 선발이던 선수가 화요일엔 벤치이고, 벤치 선수가 그날 선발로 올라갑니다.
  ...
  로스터 9명 × 주 3.3경기 = 29.7 선수-경기   (GAMES_PER_WEEK = 3.299 · 38차)
...
# ── 주간 경기수 — 단일 소스 (38차) ────────────────────────────────────────
# 확정 일정 2026-10-20 개막 ~ 2027-04-11 종료 = 174일 = 24.857주 · 82경기
#   → 팀당 주간 평균 82 / 24.857 = **3.299경기**
# 종전 3.5는 "3~4경기의 중간"이라는 어림이었고, **세 파일에 각각 하드코딩**돼 있었다
# (matchup_sim · gen_docs03 · rate_cat_leverage). 이 프로젝트가 반복해서 당한
# 「같은 값을 두 곳에 두면 반드시 갈라진다」 형태라 여기로 모았다.
GAMES_PER_WEEK = 3.299

# 몬테카를로용 추첨. {3,4} 지지집합에 4의 확률을 실어 평균을 맞춘다.
#   p = 0.299 → SD = sqrt(p(1-p)) = 0.458   (종전 균등 {3,4}는 평균 3.5 · SD 0.500)
# ⚠️ 실제 NBA 주간 일정은 2~5경기까지 퍼져 있어 SD가 더 크다. 관측된 한 주(30팀)는
#    2G 4팀 · 3G 13팀 · 4G 12팀 · 5G 1팀 → 평균 3.333 · **SD 0.745** 였다.
#    지지집합을 넓히려면 그 형태를 쓰면 된다:
#        {2: .149, 3: .433, 4: .384, 5: .033}   ← 평균 3.301 · SD 0.759
#    **미적용**이다. 근거가 n=1주이고, 24차 원칙(사전 고정 계수를 실측에 맞춰 튜닝하지
#    않는다)에 걸린다. 독립 측정에서 광폭 분포를 써도 상위 2개·하위 2개 순위는 그대로이고
#    중간 c4·c5·c6만 1%p 안쪽에서 재배열됐다 — 원래 구분이 안 되던 구간이다.
#    **지금은 평균만 실측으로 맞추고 지지집합은 유지**한다.
WEEK_GAMES_P4 = round(GAMES_PER_WEEK - 3, 3)

def draw_week_games(rng):
    """주간 경기수 추첨 (평균 GAMES_PER_WEEK). rng는 random.Random."""
    return 4 if rng.random() < WEEK_GAMES_P4 else 3
