...
def main():
    ...
    out = {"seed": SEED, "iterations": ITERS, "gp_factor": FACTOR,
           ...
           "why": ("승률 1차 지표가 상위 5개를 1.2%p 안으로 몰아넣었고 maximin 은 쓸 수 없다. "
                   "🔴 **42차에 이 표를 1차 선택 기준에서 참고 열로 강등했다** — "
                   "① 8팀/14팀 진출이라 바닥이 사실상 공짜인데 이건 바닥 지표다 "
                   "② ±0 여부는 「1순위 중 실측 비중 <50% 선수가 있는가」의 재진술이라 "
                   "**정의의 산물**이다. ⚠️ 다만 **크기**(c5 −11.3 vs c3 −2.1)는 측정이므로 "
                   "지우지 않고 참고로 남긴다. 선택 순서는 1차 조건 · 2차 조달 · "
                   "3차 상위 8팀 상대 승률 (cores.json.core_selection_hierarchy_42)."),
