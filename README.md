...
4. **비율 캣 볼륨 레버리지 계산.** 3PT%는 `docs/03` 하단(가정: 선발 7명·주간 3.5경기·팀 3PA 135),
   ...
   ~~FG%·FT%는 시도량 데이터가 없어 같은 계산을 못 했습니다~~ → **2026-08-26 계산 완료**
   (`docs/10` · `tool/rate_cat_leverage.py`). `measured_full.json`에 `FGA`·`FTA`가 전원
   있었고(14차), 빠진 것은 데이터가 아니라 계산이었습니다. 같이 드러난 것: `docs/03`의
   **팀 3PA 고정값 135가 실측 범위(77~113) 밖**이라 3PT% 표 19명이 전원 과소 계상입니다.
...
**공개 범위 — public 유지 (사용자 결정, 2026-08-27).** `cores.json`에 철수가·태우기
지명 순서·코어별 계획가가 포함돼 리그 참가자가 URL을 알면 입찰 상한과 태우기 전략이
노출된다는 점을 제시했고, 그 위험을 알고 유지하기로 했습니다. **재상신하지 마십시오.**
